...
for iter in range(4):
    ...
    cndt, vtr, national, atkr, dfndr = data_gen()
    threshold = get_threshold(cndt, atkr, dfndr)
    ...
    try:
        ...
        with plt.style.context("seaborn"):
            ...
            for i in range(1, len(record)):
                plt.hlines(record[i, 1], record[i - 1, 0], record[i, 0], colors=color[iter])
                try:
                    plt.vlines(record[i, 0], record[i, 1], record[i + 1, 1], colors=color[iter])
                except:
                    pass

            plt.hlines(record[i, 1], record[-1, 0], threshold, colors=color[iter])
